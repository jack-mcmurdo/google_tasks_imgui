...
- [ ] Delete the old, now-leaked-and-abandoned OAuth client in Cloud Console once the new one is confirmed working end-to-end (final verify step below) — no reason to leave a dead-but-still-registered client around.
- [ ] **Final step — confirm with user immediately before running any of this, it's all destructive/irreversible:** purge history *and* all previous releases in one pass, per user's "keep it clean" instruction (not just re-cutting `v2.0.0` on top of old history as originally planned):
  ...
  6. Cut exactly **one** fresh tag on the clean, rewritten history — carrying the working PKCE + Keep integration + zero-config release.yml. Version number is an open question (see below): most likely `v2.0.0` again (matches `CMakeLists.txt`'s `PROJECT_VERSION`, and nothing usable was ever actually shipped under that name), but confirm before tagging.
  7. **Verify end-to-end on the new release**: download the built `.deb`/`.tgz`, install/extract it, confirm the bundled `client_secret.json` exists with real `client_id`/`client_secret` values, then actually run the installed binary and confirm OAuth login completes with zero manual env var/file setup. This is the real acceptance test, not just a green CI run.
...
- **Version number for the one fresh tag is still an open question** — see below.
...
## Open questions
- **What version number for the one fresh tag** after all four old releases/tags are purged? Leaning `v2.0.0` (matches `CMakeLists.txt`'s current `PROJECT_VERSION`, and nothing usable ever actually shipped under that name — the published one was broken), but restarting at `v1.0.0` is equally defensible now that history is clean. Confirm before tagging.
